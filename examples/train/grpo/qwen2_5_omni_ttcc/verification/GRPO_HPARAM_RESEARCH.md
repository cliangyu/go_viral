...
**Date:** 2026-05-29.
**Author:** research pass for the RL design decision in `HEAD_PG_RL_RUN.md §6/§10`.
...
4. **Generation bugs + cold-start.** `HEAD_PG_RL_RUN` lists the `get_rope_index` generation fix and R-text cold-start SFT as costs that only exist on this path. Long-video rollouts also hit `max_length` truncation (34/168 skipped in eval).
...
- The policy distribution is *entirely* a function of `μ_z = Linear(h_last)`. Gradient reaches the backbone only through `h_last`. Two regimes:
  ...
  - **Head + backbone** (the `HEAD_PG_RL_RUN §6` target "update head+backbone"): use the **GRPO-standard tiny LR (1e-6)** for the backbone to avoid destroying the SFT features, optionally a higher LR group for the head. TRL's own VLM GRPO example uses 1e-5 with LoRA. [https://huggingface.co/docs/trl/main/en/grpo_trainer]
...
| **Reward** | β·R_rank only to start (α=0); add α·R_acc (1−IBS) **only if** within-group spread is starved. Drop R_fmt entirely (no text). | `HEAD_PG_RL_RUN §6`; the percentile floor cancels in advantage, so R_rank spread is the signal. | High |
| **Steps to signal** | Expect reward-trend signal in **~50 steps** (your own gate); held-out SRCC movement over **100–300 steps**. | `HEAD_PG_RL_RUN §9` smoke gate. | Med |
