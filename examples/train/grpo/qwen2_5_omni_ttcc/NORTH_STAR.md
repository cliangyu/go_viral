...
- ▶ NEXT: write the **head Gaussian policy-gradient RL loop** (REINFORCE + `cross_ad_reward` + KL), **smoke-gate on real rollouts** (parse N/A; **within-group R_rank std > 0**; reward↔quality corr; reward↑ over ~50 steps). Pass → scale + track held-out SRCC vs the 1−IBS control.
...
- A1 (swift allows a clean custom Trainer subclass / compute_loss override) — **VERIFIED**: override point = `swift/trainers/seq2seq_trainer.py:123 compute_loss(self, model, inputs, return_outputs, num_items_in_batch)`; trainer selected via `swift/trainers/trainer_factory.py:58 get_trainer_cls`; docs `docs/source/Customization/Architecture.md`. PLAN: subclass `Seq2SeqTrainer`, override `compute_loss` to do head-PG REINFORCE (sample G hazard-noised curves inside, reward, within-group advantage, reinforce_loss+KL); write a small train entry that reuses swift `get_model_processor`/`get_template`/dataset loading and instantiates our trainer → `.train()` (distributed/ckpt/optim reused).
