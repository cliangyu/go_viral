#!/usr/bin/env bash
# Experiment (4): GRPO until saturation.
# Full single epoch (~179 steps) from a provided SFT_CKPT. Save every 25 steps.
# All other hyperparams identical to grpo.sh.
set -euo pipefail
...
"${VENV}/bin/python" -m swift.cli.main rlhf \
    ...
    --logging_steps 2 --eval_steps 25 --save_steps 25 --save_total_limit 5 \
    --output_dir "${OUT}" \
    --deepspeed zero2 --dataloader_num_workers 2 \
    --use_vllm true --vllm_mode colocate --vllm_gpu_memory_utilization 0.35 \
    --num_generations 2 --temperature 0.4 --top_p 0.95 --beta 0.04 \
    --log_completions true \
    2>&1 | tee "${OUT}/grpo_extended.log"
